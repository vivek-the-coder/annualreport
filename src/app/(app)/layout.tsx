...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-dvh bg-slate-50">
      ...
      {/* Desktop sidebar */}
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 bg-navy-950 lg:block" aria-label="Sidebar" style={{ paddingTop: "var(--safe-top)" }}>
        {sidebar}
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Navigation menu">
          <button className="absolute inset-0 bg-navy-950/50 backdrop-blur-[2px]" aria-label="Close menu" onClick={() => setMobileOpen(false)} />
          <div
            className="animate-fade-in absolute inset-y-0 left-0 flex w-[min(20rem,88vw)] flex-col bg-navy-950 shadow-2xl"
            style={{ paddingTop: "var(--safe-top)", paddingBottom: "var(--safe-bottom)" }}
          >
            <button onClick={() => setMobileOpen(false)} aria-label="Close menu" className="absolute right-3 top-4 z-10 rounded-lg p-2 text-white/60 hover:bg-white/10">
      ...
      {/* Topbar */}
      <header
        className="fixed inset-x-0 top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur lg:left-64"
        style={{ paddingTop: "var(--safe-top)" }}
      >
        <div className="flex h-14 items-center gap-2 px-3 sm:h-16 sm:gap-3 sm:px-4 md:px-6">
          <button onClick={() => setMobileOpen(true)} aria-label="Open menu" className="shrink-0 rounded-lg p-2 text-slate-600 hover:bg-slate-100 lg:hidden">
          ...
          {/* Mobile page title */}
          <p className="min-w-0 flex-1 truncate text-sm font-bold text-navy-900 md:hidden">
            {crumbs[crumbs.length - 1] ?? "Dashboard"}
          </p>

          {/* Search — full field from sm up; icon toggle on phones */}
          <div className={cn("relative ml-auto", searchOpen ? "absolute inset-x-0 top-0 z-40 flex h-14 items-center bg-white px-3 sm:static sm:inset-auto sm:z-auto sm:h-auto sm:bg-transparent sm:px-0" : "hidden sm:block", "w-full max-w-xs sm:max-w-sm")}>
            {searchOpen && (
              <button
                type="button"
                className="mr-2 rounded-lg p-2 text-slate-500 hover:bg-slate-100 sm:hidden"
            ...
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                placeholder="Search departments, submissions…"
                className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-sm placeholder:text-slate-400 focus:border-navy-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-navy-100"
                value={query}
              ...
              {searchFocus && results.length > 0 && (
                <ul className="animate-scale-in absolute left-0 right-0 top-11 z-50 max-h-[60vh] overflow-y-auto rounded-xl border border-slate-200 bg-white py-1 shadow-xl" role="listbox">
                  {results.map((r, i) => (
                    <li key={i}>
                      <button
          ...
          <button
            ...
            className={cn("ml-auto shrink-0 rounded-lg p-2 text-slate-600 hover:bg-slate-100 sm:hidden", searchOpen && "hidden")}
            onClick={() => setSearchOpen(true)}
          >
            <Search className="h-5 w-5" aria-hidden />
          </button>

          <Link href="/notifications" aria-label={`Notifications, ${unread} unread`} className="relative shrink-0 rounded-lg p-2 text-slate-600 transition hover:bg-slate-100">
          ...
          <div className="relative shrink-0" ref={profileRef}>
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-slate-200 py-1.5 pl-1.5 pr-1.5 transition hover:bg-slate-50 sm:pr-2.5"
            >
              ...
              <span className="hidden text-left sm:block">
                <span className="block max-w-[100px] truncate text-xs font-bold text-navy-900 md:max-w-[140px]">{user.name}</span>
            ...
            {profileOpen && (
              <div role="menu" className="animate-scale-in absolute right-0 top-12 z-50 w-[min(16rem,calc(100vw-1.5rem))] rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
      ...
      <main
        id="app-main"
        className="px-3 pb-[calc(5.5rem+var(--safe-bottom))] pt-[calc(3.75rem+var(--safe-top))] sm:px-4 sm:pt-[calc(4.5rem+var(--safe-top))] md:px-6 lg:ml-64 lg:pb-12"
      >
        ...
        {nav.slice(0, 5).map((n) => {
          ...
          return (
            <Link
              ...
              aria-current={active ? "page" : undefined}
              className={cn(
                "flex min-h-14 flex-1 flex-col items-center justify-center gap-0.5 px-1 py-2 text-[10px] font-bold",
